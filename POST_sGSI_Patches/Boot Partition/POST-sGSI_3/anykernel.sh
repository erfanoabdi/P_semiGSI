...
## AnyKernel setup
# EDIFY properties
kernel.string=8.1 to P Boot Patcher by Erfan Abdi
do.devicecheck=0
do.initd=0
do.modules=1
do.cleanup=1
device.name1=XT1650-05
device.name2=GRIFFIN
device.name3=griffin
device.name4=
device.name5=
...
write_boot() {
  ...
  if [ -f "$bin/mkbootfs" ]; then
    ...
    cd $ramdisk;
    if grep -q -v 'ro.opa.eligible_device=true' default.prop; then
        echo "ro.opa.eligible_device=true" >> default.prop
    fi
  ...
  $bin/mkbootimg --kernel $kernel --ramdisk /tmp/anykernel/ramdisk-new.cpio.gz $second --cmdline "$cmdline androidboot.selinux=permissive" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff $secondoff --tags_offset $tagsoff $dtb --output /tmp/anykernel/boot-new.img;
  if [ $? != 0 ]; then
...
#replace_string fstab.qcom "wait,verify" "wait,verify" "wait";
#replace_string fstab.qcom "forceencrypt" "forceencrypt" "encryptable";

#replace_line init.mmi.rc "on property:sys.boot_completed=1 && property:ro.boot.secure_hardware=0" "on property:sys.boot_completed=1";
